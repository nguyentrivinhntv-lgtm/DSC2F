...
def get_test_transform_with_resize(size=256):
    """
...
def create_per_generator_datasets(root, transform):
    """
...
def load_dual_stream_resnet(model_path, device):
    """Load DualStreamResNet."""
    ...
    model.eval()
    print(f"  [OK] Loaded DualStreamResNet from: {model_path}")
    return model, 'dual_stream', compute_fft_spectrum


def load_dual_stream_cnn(model_path, device):
    """Load DualStreamCNN."""
    from networks.dual_stream_cnn import (
        DualStreamCNN, compute_fft_spectrum
    )
    checkpoint = torch.load(model_path, map_location='cpu', weights_only=False)
    ...
    model = model.to(device)
    model.eval()
    print(f"  [OK] Loaded DualStreamCNN from: {model_path}")
    return model, 'dual_stream', compute_fft_spectrum


def load_dual_stream_enhanced(model_path, device):
    """Load DualStreamCNNEnhanced."""
    from networks.dual_stream_enhanced import (
        DualStreamCNNEnhanced, compute_fft_spectrum
    )
    checkpoint = torch.load(model_path, map_location='cpu', weights_only=False)
    ...
    model = model.to(device)
    model.eval()
    print(f"  [OK] Loaded DualStreamCNNEnhanced from: {model_path}")
    return model, 'dual_stream', compute_fft_spectrum
...
def run_inference_dual(model, loader, device, compute_fft_fn):
    """Forward pass cho DualStreamResNetAdvanced (dual-input: rgb + fft)."""
    y_true_all, y_prob_all = [], []
    total = len(loader)
    for i, (imgs, labels) in enumerate(loader):
        if i % 10 == 0:
            print(f"    Batch {i+1}/{total}...", end='\r')
        imgs = imgs.to(device)
        fft = compute_fft_fn(imgs)
        if fft.shape != imgs.shape:
...
def evaluate_model(model_info, dataroot, device, batch_size=32, num_workers=0, max_images=0):
    """
    Chạy evaluation trên toàn bộ dataset (hoặc từng generator nếu ForenSynths).
    
    model_info: dict với keys 'type', 'model', và optionally 'compute_fft_fn'
    """
    transform = get_test_transform()

    print("  Kiểm tra kích thước ảnh mẫu...")
    datasets_map = create_per_generator_datasets(dataroot, transform)

    if max_images > 0:
        for gen_name, dataset in datasets_map.items():
            dataset.samples = dataset.samples[:max_images]
            print(f"  [QUICK TEST] Truncated {gen_name} to {max_images} images.")
...
def parse_args():
    ...
    p.add_argument('--dataroot', type=str, default='dataset/test',
                   help='Root của test dataset (chứa 0_real/, 1_fake/ hoặc subdirs)')

    # Models
    p.add_argument('--resnet_dual_path', type=str,
                   default='weights/dual_stream_resnet/best_model.pth',
                   help='Path tới DualStreamResNet checkpoint')
    p.add_argument('--dual_stream_path', type=str,
                   default='weights/dual_stream/best_model.pth',
                   help='Path tới DualStream checkpoint')
    p.add_argument('--enhanced_path', type=str,
                   default='weights/dual_stream_enhanced/best_model.pth',
                   help='Path tới DualStreamCNNEnhanced checkpoint')

    # Which models to run
    p.add_argument('--eval_resnet_dual', action='store_true', default=True,
                   help='Evaluate DualStreamResNet')
    p.add_argument('--no_resnet_dual', action='store_true',
                   help='Bỏ qua DualStreamResNet')
    p.add_argument('--eval_dual', action='store_true', default=True,
                   help='Evaluate DualStreamCNN')
    p.add_argument('--no_dual', action='store_true',
                   help='Bỏ qua DualStreamCNN')
    p.add_argument('--eval_enhanced', action='store_true', default=True,
                   help='Evaluate DualStreamCNNEnhanced')
    p.add_argument('--no_enhanced', action='store_true',
                   help='Bỏ qua DualStreamCNNEnhanced')
...
def main():
    ...
    # A) DualStreamResNet
    # -----------------------------------------------------------------------
    run_resnet_dual = args.eval_resnet_dual and not args.no_resnet_dual
    if run_resnet_dual:
        if not os.path.isfile(args.resnet_dual_path):
            print(f"[WARNING] DualStreamResNet weights not found: {args.resnet_dual_path}")
            run_resnet_dual = False

    if run_resnet_dual:
        print(f"\n" + "-"*70)
        print("  [1/3]  DualStreamResNet")
        print("-"*70)
        model, model_type, compute_fft_fn = load_dual_stream_resnet(args.resnet_dual_path, device)
        model_info = {'type': model_type, 'model': model, 'compute_fft_fn': compute_fft_fn}

        per_split, overall = evaluate_model(
            model_info, args.dataroot, device,
            batch_size=args.batch_size, num_workers=args.num_workers, max_images=args.max_images
        )

        model_name = "DualStreamResNet"
        print_table("DualStreamResNet Results", per_split, overall, model_name)
    ...
    if run_dual:
        print(f"\n" + "-"*70)
        print("  [2/3]  DualStreamCNN")
        print("-"*70)
        model, model_type, compute_fft_fn = load_dual_stream_cnn(args.dual_stream_path, device)
        model_info = {'type': model_type, 'model': model, 'compute_fft_fn': compute_fft_fn}
        ...
        model_name = "DualStreamCNN"
        print_table("DualStreamCNN Results", per_split, overall, model_name)
    ...
    # -----------------------------------------------------------------------
    # C) DualStreamCNNEnhanced
    # -----------------------------------------------------------------------
    run_enhanced = args.eval_enhanced and not args.no_enhanced
    if run_enhanced:
        if not os.path.isfile(args.enhanced_path):
            print(f"[WARNING] Enhanced weights not found: {args.enhanced_path}")
            run_enhanced = False

    if run_enhanced:
        print(f"\n" + "-"*70)
        print("  [3/3]  DualStreamCNNEnhanced")
        print("-"*70)
        model, model_type, compute_fft_fn = load_dual_stream_enhanced(
            args.enhanced_path, device
        )
        model_info = {
            'type': model_type,
            'model': model,
        ...
        model_name = "DualStreamCNNEnhanced"
        print_table("DualStreamCNNEnhanced Results", per_split, overall, model_name)
