...
def get_settings() -> Settings:
    ...
    """
    settings = Settings()
    # Cảnh báo nếu SECRET_KEY vẫn là giá trị mặc định yếu
    _weak_keys = {"your-super-secret-key-change-this-in-production", "secret", "changeme"}
    if settings.SECRET_KEY in _weak_keys or len(settings.SECRET_KEY) < 32:
        import warnings
        warnings.warn(
            "\n⚠️  SECRET_KEY quá yếu hoặc chưa được thay đổi! "
            "Hãy tạo key mạnh bằng: python -c \"import secrets; print(secrets.token_hex(32))\" "
            "và cập nhật trong file .env",
            stacklevel=2,
        )
    return settings
