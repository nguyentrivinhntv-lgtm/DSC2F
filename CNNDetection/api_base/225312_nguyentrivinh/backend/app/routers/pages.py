...
def get_pages(is_admin: bool = False):
    ...
    try:
        ...
        import logging
        logging.error(f"Error in get_pages: {str(e)}")
        raise HTTPException(status_code=500, detail="Đã xảy ra lỗi hệ thống. Vui lòng thử lại sau.")
...
def create_page(page: PageCreate, admin_user: dict = Depends(require_admin)):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=str(e)) # Có thể để lại vì thường là validation error do code mình quăng ra
    except Exception as e:
        import logging
        logging.error(f"Error in create_page: {str(e)}")
        raise HTTPException(status_code=500, detail="Đã xảy ra lỗi hệ thống. Vui lòng thử lại sau.")
...
def update_page(slug: str, page: PageUpdate, admin_user: dict = Depends(require_admin)):
    ...
    try:
        ...
        import logging
        logging.error(f"Error in update_page: {str(e)}")
        raise HTTPException(status_code=500, detail="Đã xảy ra lỗi hệ thống. Vui lòng thử lại sau.")
...
def delete_page(slug: str, admin_user: dict = Depends(require_admin)):
    ...
    try:
        ...
        import logging
        logging.error(f"Error in delete_page: {str(e)}")
        raise HTTPException(status_code=500, detail="Đã xảy ra lỗi hệ thống. Vui lòng thử lại sau.")
