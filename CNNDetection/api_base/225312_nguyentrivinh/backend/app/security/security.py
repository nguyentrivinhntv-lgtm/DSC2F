...
# HTTP Bearer scheme (auto_error=False để cho phép đọc từ cookie)
...
def get_current_user(
    ...
    """
    FastAPI dependency - Lấy user hiện tại từ JWT token trong Cookie hoặc Header.
    """
    ...
    if not token:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Không tìm thấy xác thực JWT.",
            headers={"WWW-Authenticate": "Bearer"},
        )
