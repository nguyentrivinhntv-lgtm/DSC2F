...
async function init() {
    ...
    if (flutterToken) {
        ...
        // Cookie HttpOnly set by backend
        ...
        try {
            ...
            if (meRes.ok) {
                ...
                fetch(`${API_URL || 'http://localhost:8000'}/auth/logout`, {method: 'POST', credentials: 'include'}).catch(e=>console.log(e));
    ...
    if (authUser) {
        // Hiển thị dashboard ngay lập tức bằng dữ liệu cache để tránh chớp màn hình (flicker)
        showDashboard();

        // Chạy ngầm API /auth/me để cập nhật số token mới nhất
        try {
            const meRes = await fetch(`${API_URL}/auth/me`, { credentials: 'include' });
            if (meRes.ok) {
                const meData = await meRes.json();
                ...
                // Cập nhật lại UI sau khi có dữ liệu mới
                if (tokenCount) {
                    tokenCount.innerText = authRole === 'admin' ? 'Unlimited' : authPredictionTokens;
                }
            } else {
                // Cookie đã hết hạn hoặc không hợp lệ -> đá ra ngoài
                showAuth();
            }
        } catch(e) {
            console.error('Failed to fetch user profile:', e);
...
async function handleHybridGoogleLogin() {
    ...
    try {
        ...
        const initRes = await fetch(`${API_URL}/auth/login-session`, { credentials: 'include',  method: 'POST', body: formData });
        if (!initRes.ok) {
        ...
        const pollInterval = setInterval(async () => {
            ...
            try {
                ...
                if (data.status === 'completed' && data.token) {
                    ...
                    try {
                        const topupRes = await fetch(`${API_URL}/users/me/prediction-tokens`, { credentials: 'include', 
                            headers: {  }
                        });
                        if (topupRes.ok) {
...
function syncApiTokenPreview() {
    ...
    apiTokenPreview.innerText = `[Đã mã hóa và bảo mật trong HttpOnly Cookie]`;
}
...
async function requestRegisterOtp() {
    ...
    try {
        const res = await fetch(`${API_URL}/auth/request-register`, { credentials: 'include', 
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
analyzeBtn.addEventListener('click', async () => {
    ...
    try {
        const res = await fetch(`${API_URL}/predict`, { credentials: 'include', 
            method: 'POST',
            headers: {
                 // Token xác thực
            },
...
async function fetchHistory(options = {}) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (updateHistoryTable) {
                ...
                cachedHistoryItems.forEach(item => {
                    ...
                    html += `
                        <tr>
                            <td>${timeStr}</td>
                            <td><i class="fa-solid fa-user" style="color:var(--text-muted)"></i> ${window.escapeHTML(item.username)}</td>
                            <td title="${window.escapeHTML(item.filename)}">${window.escapeHTML(item.filename).length > 20 ? window.escapeHTML(item.filename).substring(0, 20) + '...' : window.escapeHTML(item.filename)}</td>
            ...
            tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;color:var(--danger)">Lỗi: ${data.detail.replace(/</g, '&lt;')}</td></tr>`;
        }
    } catch (e) {
        if (updateHistoryTable) {
...
async function runBatchScan() {
    ...
    try {
        const res = await fetch(`${API_URL}/predict/batch`, { credentials: 'include', 
            method: 'POST',
            headers: {
        ...
        batchResultTbody.innerHTML = results.map((item) => {
            if (item.label === 'error') {
                return `
                    <tr>
                        <td>${window.escapeHTML(item.filename)}</td>
            ...
            return `
                <tr>
                    <td title="${window.escapeHTML(item.filename)}">${window.escapeHTML(item.filename)}</td>
                    <td>${MODEL_NAMES[item.model_used] || item.model_used}</td>
...
async function testApiEndpoint() {
    ...
    try {
        const res = await fetch(`${API_URL}${endpoint}`, { credentials: 'include',  headers });
...
async function payVNPay(packageName, tokens) {
    try {
        ...
        const response = await fetch(`${API_URL}/payment/create_payment_url`, { credentials: 'include', 
            method: "POST",
            headers: {
...
async function renderPricingGrid() {
    ...
    try {
        ...
        gridContainer.innerHTML = packages.map(pkg => `
            <div class="pricing-card ${pkg.popular ? 'popular' : ''}" onclick="payVNPay('${pkg.name}', ${pkg.tokens || 0})">
...
if (profileBtn && profileModal) {
    profileBtn.addEventListener('click', () => {
        ...
        document.getElementById('profile-username').innerText = authUser || 'Unknown';
...
if (btnChangePassword) {
    btnChangePassword.addEventListener('click', async () => {
        ...
        try {
            ...
            console.error(e);
            errEl.innerText = "Lỗi kết nối: " + e.message;
            errEl.classList.remove('hidden');
        } finally {
...
async function fetchPaymentHistory() {
    ...
    try {
        const res = await fetch(`${API_URL}/payment-history/`, { credentials: 'include', 
            headers: {
        ...
        if (res.ok) {
            ...
            data.items.forEach(item => {
                ...
                const btnViewEl = tr.querySelector('.btn-view-order');
...
function initUserNotifications() {
    ...
    if (btnMarkAllRead) {
        btnMarkAllRead.addEventListener('click', async () => {
            try {
                const res = await fetch(`${API_URL}/notifications/read-all`, { credentials: 'include', 
                    method: 'PUT',
                    headers: {  }
                });
                if (res.ok) {
...
async function markNotificationRead(id) {
    try {
        const res = await fetch(`${API_URL}/notifications/${id}/read`, { credentials: 'include', 
            method: 'PUT',
            headers: {  }
        });
        if (res.ok) {
