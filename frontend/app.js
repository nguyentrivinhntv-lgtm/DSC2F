    ...
    try {
        ...
        if (res.ok) {
            ...
            if (data.items.length === 0) {
                tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:var(--text-muted)">Chua c� giao d?ch mua h�ng n�o.</td></tr>';
                return;
            }

            tbody.innerHTML = '';
            $totalSpent = data.items.reduce((sum, item) => sum + item.amount, 0); $totalTokens = data.items.reduce((sum, item) => sum + item.tokens, 0); if (document.getElementById('user-total-spent')) document.getElementById('user-total-spent').innerText = $totalSpent.toLocaleString('vi-VN') + ' d'; if (document.getElementById('user-total-tokens-bought')) document.getElementById('user-total-tokens-bought').innerText = $totalTokens.toLocaleString('vi-VN'); data.items.forEach(item => {
                const tr = document.createElement('tr');
                tr.innerHTML = `n                    <td></td>
                    <td></td>
                    <td> d</td>
                    <td style="color:var(--primary);font-weight:bold;">+</td>
                    <td><span class="badge" style="background:#ecfdf5;color:#065f46;border-color:#86efac;">Th�nh c�ng</span></td>
                ;
                tbody.appendChild(tr);
            });
        } else {
            if (tbody) tbody.innerHTML = <tr><td colspan="5" style="text-align:center;color:var(--danger)">L?i t?i d? li?u: </td></tr>;
        }
    } catch (err) {
        console.error(err);
        if (tbody) tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:var(--danger)">Kh�ng th? k?t n?i API.</td></tr>';
    }
}
